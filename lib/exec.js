  ...
  dispose() {
    ...
    if (this.child) {
      ...
      this.child.on("error", (error) => {
        if (error && error.code !== "ERR_IPC_DISCONNECTED") {
          log("ESLint worker error after dispose:", String(error.message || error));
        }
      });
      ...
      try {
        ...
        if (error && error.code !== "ERR_IPC_DISCONNECTED") {
          log("ESLint worker disconnect error:", String(error.message || error));
        }
      }
