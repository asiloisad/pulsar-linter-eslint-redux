...
// Projects with no ESLint config (tried both v8 and v10) - skip linting entirely
...
function getEngine(projectRoot) {
  ...
  startBusyMessage(projectRoot, "Loading ESLint");
  const promise = resolveEslint(projectRoot)
    .then((resolved) => {
...
async function tryAlternateVersion(projectRoot, currentSource) {
  const altVersion = currentSource === "bundled-v8" ? "v10" : "v8";
        ...
        if (altInfo) {
          try {
            ...
            if (isNoConfigError(altError)) {
              ...
              log("No ESLint config found (tried both v8 and v10), skipping project");
            } else {
...
module.exports = { exec, handle, resetEngine, getEngine, setBusySignal };
